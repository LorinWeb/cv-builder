...
export function ResumeStudioDialog({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenStateChange}>
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-50 bg-(--color-surface-overlay) backdrop-blur-[2px]" />
        <Dialog.Popup
          data-testid="resume-studio-dialog"
          className="fixed inset-4 z-60 mx-auto my-6 flex max-h-[calc(100vh-48px)] w-[min(1040px,100%)] flex-col overflow-hidden rounded-4xl bg-[linear-gradient(180deg,var(--color-surface-modal-start),var(--color-surface-modal-end))] shadow-[0_40px_120px_-48px_rgba(11,37,31,0.55)]"
        >
          <div className="flex items-start justify-between gap-4 border-b border-(--color-border-subtle) px-5 py-4 md:px-6">
            <div>
              <Dialog.Title className="m-0 text-xl font-medium text-(--color-text-strong)">
                Resume Studio
              </Dialog.Title>
              {state.activeVersionName ? (
                <p className="mt-2 text-xs font-medium uppercase tracking-[0.08em] text-(--color-text-muted)">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 md:px-6">
            {statusMessage ? (
              <p className="mb-4 rounded-2xl border border-(--color-border-success) bg-(--color-surface-success) px-4 py-3 text-sm text-(--color-text-success)">
                {statusMessage}
              </p>
            ) : null}
            {errorMessage ? (
          ...
          <div className="sticky bottom-0 border-t border-(--color-border-subtle) bg-[linear-gradient(180deg,var(--color-surface-modal-start),var(--color-surface-modal-end))] px-5 py-4 md:px-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p
                data-testid="resume-studio-autosave-status"
                className="m-0 text-sm font-medium text-(--color-text-strong)"
              >
                {autosaveStatusLabel}
              </p>
